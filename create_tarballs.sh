...
#---------------------------------------------------------------------------
			...
			do
				...
				do
					...
					#read var
					make -s clean
					make -s -f Makefile.SCMM
					mv single_core_MIPS_mem.tar tarballs/core${CNUM}/core${CNUM}_bank${L2BANK}_cpuop${NCPUOPS}_optmem${NUMOPT}.tar
					echo "Done. Created core${CNUM}_bank${L2BANK}_cpuop${NCPUOPS}_optmem${NUMOPT}.tar"				
				done
			done
		fi
